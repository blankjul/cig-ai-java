...
At the figure~\ref{fig:safe_grid} you can see the action that could be excluded without doing any simulation at the game.
...
This two classes follows the singleton pattern and could be created by using the factory (Figure~\ref{fig:sim_classes}).
...
select for 50 \% the action of the first or the second individual (Figure ~\ref{fig:crossover}).
...
For that we use the principle of a sliding window (Figure~\ref{fig:sliding_window}). The first
