...
The theory of \ac{MCTS} is described in section \ref{sec:back}. The standard approach was modified to fit our problem. The whole \ac{MCTS} algorithm is computed in the class \textit{MCTSStrategy} which is used by the \textit{MCTSAgent} and the \textit{MCTSHeuristicAgent}.
...
\todo{not sure if this is right: We chose this approach because at the begnning we have no information about the node and its neighbourhood so we have to compute more simulations to get a better overview. At the end, when the tree is bulid-up for the most part, only a few simulations are needed because we already know whether this is most likely a good or abad node.}
